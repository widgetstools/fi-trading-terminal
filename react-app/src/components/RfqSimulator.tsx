  ...
  return (
    <div style={{ position: 'relative', width: 28, height: 28, flexShrink: 0 }}>
      <svg viewBox="0 0 28 28" width={28} height={28}>
        <circle cx="14" cy="14" r={R} fill="none" stroke="var(--bn-bg3)" strokeWidth="2.5" />
...
const thStyle: React.CSSProperties = {
  fontSize: 9, color: 'var(--bn-t1)', textTransform: 'uppercase', letterSpacing: '0.04em',
  fontWeight: 600, padding: '6px 8px', textAlign: 'left', whiteSpace: 'nowrap',
  background: 'var(--bn-bg2)', borderBottom: '1px solid var(--bn-border)',
};
...
const cellBase: React.CSSProperties = {
  ...
  borderBottom: '1px solid var(--bn-border)', whiteSpace: 'nowrap',
};
        ...
        {sorted.map(q => {
          ...
          return (
            <tr key={q.dealer} style={{ opacity: rowOpacity }}>
              <td style={{ ...cellBase, fontWeight: 700, color: 'var(--bn-cyan)' }}>{q.dealer}</td>
              <td style={{ ...cellBase, textAlign: 'right', fontWeight: isBestBid ? 700 : 400, color: isBestBid ? 'var(--bn-blue)' : '#5a7090' }}>
                {q.bid.toFixed(3)}
                {isBestBid && <span style={{ marginLeft: 4, fontSize: 9, fontWeight: 700, color: 'var(--bn-blue)' }}>▲BEST</span>}
              </td>
              <td style={{ ...cellBase, textAlign: 'right', color: 'var(--bn-t1)' }}>{q.bidSize}</td>
              <td style={{ ...cellBase, textAlign: 'right', fontWeight: isBestAsk ? 700 : 400, color: isBestAsk ? 'var(--bn-red)' : '#7a4050' }}>
                {q.ask.toFixed(3)}
              ...
              <td style={cellBase}>
                {isDone ? (
                  <CheckCircle size={14} style={{ color: 'var(--bn-green)' }} />
                  ...
                  <div className="flex gap-1.5">
                    <button onClick={() => onHitLift(rfqId, q.dealer, 'hit')}
                      ...
                      style={{ fontSize: 11, background: 'rgba(61,158,255,0.15)', color: 'var(--bn-blue)', border: '1px solid rgba(61,158,255,0.35)' }}>
                      HIT
                    </button>
                    <button onClick={() => onHitLift(rfqId, q.dealer, 'lift')}
                      className="font-mono-fi px-2.5 py-1 rounded-sm font-bold"
                      style={{ fontSize: 11, background: 'rgba(0,229,160,0.15)', color: 'var(--bn-green)', border: '1px solid rgba(0,229,160,0.35)' }}>
                      LIFT
                    </button>
                  </div>
                ) : null}
    ...
    <div className="flex flex-col h-full" style={{ background: 'var(--bn-bg1)' }}>
      ...
      <div className="flex flex-1 overflow-hidden gap-px" style={{ background: 'var(--bn-border)' }}>
        ...
        <div className="flex flex-col flex-shrink-0" style={{ width: 220, minWidth: 180, background: 'var(--bn-bg2)' }}>
            ...
            <div ref={instrRef} style={{position:'relative'}}>
              ...
              {activeBond && !instrOpen ? (
                ...
                <div className="relative">
                  <Search size={10} className="absolute left-2 top-1/2 -translate-y-1/2" style={{color:'var(--bn-t2)'}}/>
                  <input
                    autoFocus
                    value={instrSearch}
              ...
              {instrOpen && instrResults.length > 0 && (
                <div className="absolute left-0 right-0 z-50 rounded-sm border overflow-hidden"
                  style={{top:'100%',marginTop:2,background:'var(--bn-bg2)',borderColor:'var(--bn-border2)',maxHeight:200,overflowY:'auto'}}>
                  {instrResults.map(b => (
                    <div key={b.id} onClick={() => pickBond(b)}
                      className="flex items-center justify-between px-2 py-1.5 cursor-pointer transition-colors"
                      style={{borderBottom:'1px solid var(--bn-border)',fontSize:9,fontFamily:'JetBrains Mono,monospace'}}
                      onMouseEnter={e => (e.currentTarget.style.background = 'var(--bn-bg3)')}
                      onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                      <div>
                        <span style={{color:'var(--bn-cyan)',fontWeight:700}}>{b.ticker}</span>
                        <span style={{color:'var(--bn-t1)',marginLeft:4}}>{b.cpn} {b.mat}</span>
                      </div>
              ...
              {instrOpen && instrSearch.length > 0 && instrResults.length === 0 && (
                <div className="absolute left-0 right-0 z-50 rounded-sm border px-2 py-2 font-mono-fi"
                  style={{top:'100%',marginTop:2,background:'var(--bn-bg2)',borderColor:'var(--bn-border2)',fontSize:9,color:'var(--bn-t2)'}}>
                  No matches
                </div>
              )}
            </div>
          ...
          {/* RFQ history list */}
          <div className="border-t flex-1 overflow-y-auto" style={{ borderColor: 'var(--bn-border)' }}>
            <div className="flex items-center justify-between px-3 h-7 border-b" style={{ borderColor: 'var(--bn-border)' }}>
            ...
            {requests.map(r => (
              <div key={r.id} onClick={() => setActiveId(r.id)}
                ...
                style={{ borderColor: 'var(--bn-border)', background: activeId === r.id ? 'var(--bn-bg3)' : 'transparent' }}>
                <div className="flex items-center justify-between mb-0.5">
                  <div className="flex items-center gap-2">
                    <span className="font-mono-fi font-bold" style={{ fontSize: 9, color: 'var(--bn-cyan)' }}>{r.id}</span>
                ...
                <div className="font-mono-fi" style={{ fontSize: 9, color: 'var(--bn-t1)' }}>{r.bond}</div>
                <div className="flex items-center justify-between">
                  <span className="font-mono-fi" style={{ fontSize:9, color: 'var(--bn-t2)' }}>{r.size} · {r.quotes.length} quotes</span>
        ...
        <div className="flex flex-col flex-1 overflow-hidden" style={{ background: 'var(--bn-bg1)' }}>
          {activeReq ? (
            <>
              <div className="flex items-center justify-between px-3 h-8 border-b flex-shrink-0" style={{ borderColor: 'var(--bn-border)' }}>
                <div className="flex items-center gap-3">
                  <span className="ph-title">{activeReq.id}</span>
                  <span className="font-mono-fi font-bold" style={{ fontSize: 11, color: 'var(--bn-cyan)' }}>{activeReq.bond}</span>
                  <span className="font-mono-fi" style={{ fontSize:11, color: activeReq.side === 'Buy' ? 'var(--bn-green)' : 'var(--bn-red)', fontWeight: 700 }}>{activeReq.side.toUpperCase()}</span>
                  <span className="font-mono-fi" style={{ fontSize:11, color: 'var(--bn-t1)' }}>{activeReq.size}</span>
                </div>
              ...
              {activeReq.quotes.length > 0 && activeReq.status !== 'done' && (
                <div className="flex items-center gap-6 px-4 py-2 border-b flex-shrink-0" style={{ background: 'var(--bn-bg2)', borderColor: 'var(--bn-border)' }}>
                  <div>
                    <div className="col-hdr mb-0.5">Best Bid</div>
                    <div className="font-mono-fi font-bold" style={{ fontSize:13, color: 'var(--bn-blue)' }}>{bestBid.toFixed(3)}</div>
                  </div>
                  <div>
                    <div className="col-hdr mb-0.5">Best Ask</div>
                    <div className="font-mono-fi font-bold" style={{ fontSize:13, color: 'var(--bn-red)' }}>{bestAsk.toFixed(3)}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="font-mono-fi font-bold" style={{ fontSize:13, color: 'var(--bn-green)' }}>{activeReq.quotes.length}</div>
                  </div>
                </div>
              )}
              ...
              {activeReq.status === 'done' && executedTrade && executedTrade.rfqId === activeReq.id && (
                <div className="flex items-center gap-3 px-4 py-2.5 border-t flex-shrink-0" style={{ borderColor: 'var(--bn-border)', background: 'rgba(45,212,191,0.06)' }}>
                  <CheckCircle size={14} style={{ color: 'var(--bn-green)' }} />
                  <div>
                    <span className="font-mono-fi font-bold" style={{ fontSize:11, color: 'var(--bn-green)' }}>
                      {executedTrade.side} {executedTrade.size} @ {executedTrade.price.toFixed(3)}
                    </span>
                    <span className="font-mono-fi" style={{ fontSize:9, color: 'var(--bn-t1)', marginLeft: 8 }}>
              ...
              {activeReq.status === 'done' && (!executedTrade || executedTrade.rfqId !== activeReq.id) && (
                <div className="flex items-center gap-3 px-4 py-2 border-t flex-shrink-0" style={{ borderColor: 'var(--bn-border)', background: 'rgba(45,212,191,0.05)' }}>
                  <CheckCircle size={14} style={{ color: 'var(--bn-green)' }} />
                  <span className="font-mono-fi font-bold" style={{ fontSize:11, color: 'var(--bn-green)' }}>RFQ COMPLETE</span>
                </div>
              )}
            </>
          ) : (
