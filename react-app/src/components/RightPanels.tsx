    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-3 h-8 border-b flex-shrink-0" style={{background:'var(--bn-bg1)',borderColor:'var(--bn-border)'}}>
        <div>
          ...
          <span className="font-mono-fi ml-2" style={{fontSize:9,color:'var(--bn-t2)'}}>UST · Aaa · Gov</span>
        </div>
        <div className="flex gap-1">
          {['Par','YTM','DV01'].map((v,i)=>(
            <button key={v} className="pact" style={{background:i===0?'rgba(61,158,255,0.1)':'transparent',borderColor:i===0?'var(--bn-blue)':'var(--bn-border2)',color:i===0?'var(--bn-blue)':'var(--bn-t2)'}}>{v}</button>
          ))}
        </div>
      </div>
      <div className="flex-1 overflow-y-auto">
          ...
          <thead>
            <tr style={{background:'var(--bn-bg2)',position:'sticky',top:0,zIndex:1}}>
              {['CPN','MATURITY','BID','ASK','YTM','SPD','AMT','AXES'].map(h=>(
                <th key={h} className={`col-hdr px-2 py-1.5 border-b ${h==='CPN'||h==='MATURITY'?'text-left':'text-right'}`} style={{borderColor:'var(--bn-border)'}}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {UST_LADDER.map((r,i)=>(
              <tr key={i} className="border-b hover:bg-[var(--bn-bg3)] cursor-pointer" style={{borderColor:'var(--bn-border)'}}>
                ...
                <td className="font-mono-fi px-2 py-1 text-left" style={{fontSize:11,color:'var(--bn-t1)'}}>{r.maturity}</td>
                <td className="font-mono-fi px-2 py-1 text-right" style={{fontSize:11,color:'var(--bn-blue)'}}>{r.bid.toFixed(3)}</td>
                <td className="font-mono-fi px-2 py-1 text-right" style={{fontSize:11,color:'var(--bn-red)'}}>{r.ask.toFixed(3)}</td>
...
export function OrderTicket({ selectedBond, onSendRfq }: OrderTicketProps) {
  return (
    <div className="flex flex-col" style={{borderTop:`1px solid var(--bn-border)`}}>
      <div className="flex items-center justify-between px-3 h-8 border-b" style={{background:'var(--bn-bg1)',borderColor:'var(--bn-border)'}}>
        <span className="ph-title">Order Ticket</span>
        {selectedBond && <span className="font-mono-fi" style={{fontSize:9,color:'var(--bn-cyan)'}}>{selectedBond.ticker} {selectedBond.cpn} {selectedBond.mat}</span>}
      </div>
      <div className="p-3 flex flex-col gap-2">
        <div className="grid grid-cols-2 gap-1.5">
          <button className="font-mono-fi py-1.5 rounded-sm border font-bold tracking-widest" style={{fontSize:11,background:'rgba(0,229,160,0.1)',borderColor:'rgba(0,229,160,0.3)',color:'var(--bn-green)'}}>BUY</button>
          <button className="font-mono-fi py-1.5 rounded-sm border font-bold tracking-widest" style={{fontSize:11,background:'transparent',borderColor:'var(--bn-border2)',color:'var(--bn-t2)'}}>SELL</button>
        </div>
        {[
