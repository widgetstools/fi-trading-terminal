...
const RTG_COLOR: Record<string, string> = { aaa: '#42b0df', aa: '#e9c230', a: '#a8d229', bbb: '#ec8039', hy: '#db3880' };
...
const SECTOR_COLORS = ['#42b0df', '#e9c230', '#9b7fc5', '#a8d229', '#ec8039', '#b65420'];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bn-bg1)', overflow: 'hidden' }}>
      <div style={{ flex: 1 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={BUCKET_DATA} margin={{ top: 12, right: 16, bottom: 4, left: 4 }}>
            ...
            <Tooltip content={<TT />} />
            <Bar yAxisId="dv01" dataKey="dv01" name="DV01" fill="#42b0df" radius={[3, 3, 0, 0]} barSize={18} fillOpacity={0.8} />
            <Bar yAxisId="oas" dataKey="avgOas" name="Avg OAS" fill="#ec8039" radius={[3, 3, 0, 0]} barSize={14} fillOpacity={0.7} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      {/* Summary footer */}
      <div style={{ display: 'flex', gap: 14, padding: '6px 14px', borderTop: BD, flexShrink: 0 }}>
        {[
          { l: 'Total DV01', v: `$${(totalDv01All / 1000).toFixed(1)}K`, c: '#42b0df' },
          { l: 'Avg Dur', v: '4.82yr', c: '#e9c230' },
          { l: 'Bonds', v: String(BUCKET_DATA.reduce((a, d) => a + d.bonds, 0)), c: 'var(--bn-t0)' },
          { l: 'Wt Avg OAS', v: `+${Math.round(BUCKET_DATA.reduce((a, d) => a + d.avgOas * d.bonds, 0) / BUCKET_DATA.reduce((a, d) => a + d.bonds, 0))}bp`, c: '#ec8039' },
        ].map(s => (
      ...
      <div style={{ flex: 1 }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={HIST_OAS} margin={{ top: 4, right: 16, bottom: 8, left: 8 }}>
            <defs>
              <linearGradient id="igGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#42b0df" stopOpacity={0.15} /><stop offset="95%" stopColor="#42b0df" stopOpacity={0} /></linearGradient>
            ...
            <Tooltip content={<TT />} />
            <Area yAxisId="ig" type="monotone" dataKey="ig" name="CDX IG" stroke="#42b0df" strokeWidth={1.5} fill="url(#igGrad)" dot={false} />
            ...
            <Tooltip content={(props: any) => {
              ...
              return (
                <div style={{ background: 'var(--bn-bg2)', border: BD, borderRadius: 3, padding: '6px 10px', fontFamily: 'JetBrains Mono,monospace', fontSize: 11 }}>
                  ...
                  <div style={{ color: d.payload.color || '#42b0df' }}>OAS: +{d.value}bp</div>
                </div>
              );
            }} />
...
export function PnlAttribution() {
  const chartData = [
    ...PNL_DATA.map(d => ({ ...d, fill: d.pnl >= 0 ? '#42b0df' : 'var(--bn-red)' })),
            ...
            <Tooltip content={(props: any) => {
              ...
              return (
                <div style={{ background: 'var(--bn-bg2)', border: BD, borderRadius: 3, padding: '6px 10px', fontFamily: 'JetBrains Mono,monospace', fontSize: 11 }}>
                  ...
                  <div style={{ color: isPos ? '#42b0df' : 'var(--bn-red)', fontWeight: 600 }}>{isPos ? '+' : ''}${d.value}K</div>
                </div>
              );
            }} />
