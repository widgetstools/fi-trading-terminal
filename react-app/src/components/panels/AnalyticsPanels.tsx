...
const RTG_COLOR: Record<string, string> = { aaa: '#e32636', aa: '#c01020', a: '#00a676', bbb: '#ff6b35', hy: '#e63980' };
...
const SECTOR_COLORS = ['#e32636', '#c01020', '#8b5cf6', '#00a676', '#ff6b35', '#d15427'];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bn-bg1)', overflow: 'hidden' }}>
      <div style={{ flex: 1 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={BUCKET_DATA} margin={{ top: 12, right: 16, bottom: 4, left: 4 }}>
            ...
            <Tooltip content={<TT />} />
            <Bar yAxisId="dv01" dataKey="dv01" name="DV01" fill="#e32636" radius={[3, 3, 0, 0]} barSize={18} fillOpacity={0.8} />
            <Bar yAxisId="oas" dataKey="avgOas" name="Avg OAS" fill="#ff6b35" radius={[3, 3, 0, 0]} barSize={14} fillOpacity={0.7} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      {/* Summary footer */}
      <div style={{ display: 'flex', gap: 14, padding: '6px 14px', borderTop: BD, flexShrink: 0 }}>
        {[
          ...
          { l: 'Wt Avg OAS', v: `+${Math.round(BUCKET_DATA.reduce((a, d) => a + d.avgOas * d.bonds, 0) / BUCKET_DATA.reduce((a, d) => a + d.bonds, 0))}bp`, c: '#ff6b35' },
        ].map(s => (
      ...
      <div style={{ flex: 1 }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={HIST_OAS} margin={{ top: 4, right: 16, bottom: 8, left: 8 }}>
            <defs>
              <linearGradient id="igGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#e32636" stopOpacity={0.15} /><stop offset="95%" stopColor="#e32636" stopOpacity={0} /></linearGradient>
            ...
            <Tooltip content={<TT />} />
            <Area yAxisId="ig" type="monotone" dataKey="ig" name="CDX IG" stroke="#e32636" strokeWidth={1.5} fill="url(#igGrad)" dot={false} />
            ...
            <Tooltip content={(props: any) => {
              ...
              return (
                <div style={{ background: 'var(--bn-bg2)', border: BD, borderRadius: 3, padding: '6px 10px', fontFamily: 'JetBrains Mono,monospace', fontSize: 11 }}>
                  ...
                  <div style={{ color: d.payload.color || '#e32636' }}>OAS: +{d.value}bp</div>
                </div>
              );
            }} />
...
export function PnlAttribution() {
  const chartData = [
    ...PNL_DATA.map(d => ({ ...d, fill: d.pnl >= 0 ? '#e32636' : 'var(--bn-red)' })),
            ...
            <Tooltip content={(props: any) => {
              ...
              return (
                <div style={{ background: 'var(--bn-bg2)', border: BD, borderRadius: 3, padding: '6px 10px', fontFamily: 'JetBrains Mono,monospace', fontSize: 11 }}>
                  ...
                  <div style={{ color: isPos ? '#e32636' : 'var(--bn-red)', fontWeight: 600 }}>{isPos ? '+' : ''}${d.value}K</div>
                </div>
              );
            }} />
